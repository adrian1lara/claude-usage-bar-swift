...
enum MenuBarIcon {
    ...
    static func signedOutImage() -> NSImage {
        ...
        let img = NSImage(systemSymbolName: "person.crop.circle.badge.exclamationmark",
    ...
    static func image(percent: Int?) -> NSImage {
        ...
        let base = NSFont.systemFont(ofSize: 10, weight: .semibold)
        let font = base.fontDescriptor.withDesign(.rounded)
            .flatMap { NSFont(descriptor: $0, size: 10) } ?? base
        let attrs: [NSAttributedString.Key: Any] = [.font: font, .foregroundColor: NSColor.black]
        let textSize = (label as NSString).size(withAttributes: attrs)

        let width = max(26, ceil(textSize.width) + 4)
        ...
        let textRect = NSRect(x: (width - textSize.width) / 2,
